...
def SCurve_query(country : str) -> DataFrame:
    ...
    if country != 'world':
        query = f"""
                ...
                INNER JOIN
                    BEV_table 
                ON BEV_table.date = total_table.date
                ORDER BY 
                    date ASC;
                """
        df_total = sql.from_sql_to_pandas(query).set_index('date').sort_index()
    else:
        query = f"""SELECT * FROM `world_scurve`"""        
        df_total = sql.from_sql_to_pandas(query).set_index('date').sort_index()
    return df_total
        ...
        for country in selected_countries:
            for months_back in months_back_list:
                ...
                if months_back !=0:
                    ...
                    fig,df = SCurve_plot(fig,df,rolling_window=rolling_window,country=country,color_dict=color_dict,month_label=(datetime.today() - relativedelta(months=months_back+1)).strftime("%b %Y"))
                else:
                    fig,df = SCurve_plot(fig,df,rolling_window=rolling_window,country=country,color_dict=color_dict,month_label=False)
                df_list.append(df)
...
def update_switch_output(type_dataset,switch_values,controller):

    """
    Update switch outputs based on user selections.
    """

    controller = [ html.Label('Rolling Window:',className = 'dropdown-label'),
                dcc.Dropdown(
    ...
    if 'dataset-dropdown' == triggered_id:
        ...
        if type_dataset in [2,4]: 
            ...
            if type_dataset == 2:
                ...
                controller += [
                        dbc.Col([
                                    html.Label('Fuel Type:',className = 'dropdown-label'),
                                    dcc.Dropdown(
                                        id={
                                            'type': 'dynamic-option',
                                            'index': 'fuel-dyn'
