...
# Monthly New Registrations Data
@cache.memoize(timeout = 3600)
...
# this callback function is related to the updating of plots and CSV download functionality
@app.callback(
    [
     Output('graph', 'figure'),
...
def update_graph(type_dataset,
    ...
    if len(selected_countries) == 0:
        return NO_PLOT, no_update, no_update, no_update

    df_list = []

    # Monthly New Registrations Plot
    if type_dataset == 1:
        ...
        fig.update_layout(generate_plotly_layout(title = title,
                                                 ...
                                                 range = [0, max_value]))
        df_list.append(df)
        csv_filename = f'{country.lower()}_national_car_sales_rw{rolling_window}.csv'

    # Manufacturers - Comparison Plot
    elif type_dataset == 2:
        missing_countries = []
        ...
        csv_filename = f'{make.lower()}_{fuelType}_{"_".join(selected_countries)}_rw{rolling_window}.csv'
    ...
    if type_dataset == 3:
        ...
        selected_countries = [country.lower() for country in selected_countries]
        csv_filename = f'{"_".join(selected_countries)}_s_curve_like_adoption_rw{rolling_window}.csv'

    # Top BEV Manufacturers Plot
    elif type_dataset == 4:
        country = selected_countries[0]
        ...
        df_list.append(df)
        csv_filename = f'{country.lower()}_top_BEV_manufacturers_rw{rolling_window}.csv'

    df_csv = pd.concat(df_list)

    if callback_context.triggered_id == 'download-button':
        return fig, dcc.send_data_frame(df_csv.to_csv, csv_filename), is_popup, message_popup
    else:
        return fig, no_update, is_popup, message_popup

# this callback function is related to the visibility of control panel options
@app.callback(
...
# this callback function is related to the visibility of the download button
@app.callback(
...
def toggle_download_button_visibility(country_values, 
                                      figure):
    if figure and figure != NO_PLOT and any(country_values):
